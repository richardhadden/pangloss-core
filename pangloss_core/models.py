...
from annotated_types import MaxLen, MinLen, BaseMetadata
from typing import Any, Optional, Annotated, Sequence, Generic, Union, TypeVar, ClassVar
from typing_extensions import Unpack
from pydantic import BaseModel as PydanticBaseModel, create_model as pydantic_create_model
...
class BaseNodeReference(PydanticBaseModel):
    __abstract__ = True
...
class BaseNode(BaseNodeReference):
    ...
    def __pydantic_init_subclass__(cls, **kwargs: dict[str, Any]) -> None:
        ...
        #ic(cls, cls.__pros_traits_as_direct_ancestor__())
        #ic(cls, cls.__pros_traits_as_indirect_ancestor__)
        cls.__pg_delete_indirect_trait_fields__()
        cls.model_rebuild(force=True)
        cls.Reference = type(f"{cls.__name__}Reference", (BaseNodeReference,), {})
    ...
    def __pg_model_labels__(cls) -> list[str]:
        ...
        return [c.__name__ for c in cls.mro() if (issubclass(c, BaseNode) and c is not BaseNode) or c in cls.__pg_traits_as_direct_ancestors__]
    ...
    def __pg_traits_as_indirect_ancestors__(cls) -> set[AbstractTrait]:
        ...
        for c in cls.mro():
            if issubclass(c, AbstractTrait) and not issubclass(c, BaseNode) and c is not AbstractTrait and c is not cls and c not in traits_as_direct_ancestors:
...
class RelationModel(PydanticBaseModel):
    pass

@dataclass
class RelationConfig:
    reverse_name: str
    relation_to_base: Optional[type[BaseNode]] = None
    relation_model: Optional[RelationModel] = None
    validators: Optional[Sequence[BaseMetadata]] = None
    
    
RelationToType = TypeVar("RelationToType", type[BaseNode], type[BaseNode])

class RelationTo(PydanticBaseModel):
    ...
    def __class_getitem__(cls, args: tuple[RelationToType, RelationConfig]) -> set[type[RelationToType]]:
        """Creates a Pydantic-friendly Annotated type """
        try:
            related_type: type[BaseNode] = args[0]
        except TypeError:
            raise PanglossConfigError("A RelationConfig instance must be provided as part of the type annotation")
        
        relation_config: RelationConfig = args[1]
        
        # Set the relation_config relation_to_base to the actual class concerned
        relation_config.relation_to_base = related_type
        
        # Get the subclasses of related-to cls, as possible allowed types
        related_types = tuple([related_type.Reference, *[rt.Reference for rt in related_type.__pg_all_subclasses__()]])
...
class AbstractMixin(PydanticBaseModel):
    pass
