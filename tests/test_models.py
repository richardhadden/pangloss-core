...
import pytest
from pangloss_core.models import BaseNode, AbstractTrait, RelationTo, RelationConfig, RELATION_IDENTIFIER
...
def test_traits_are_not_inherited_indirectly():
    ...
    assert set(NonPurchaseablePet.__pg_model_labels__) == set(["NonPurchaseablePet", "Animal", "Pet"])
...
def test_relation_to():
    ...
    class Person(BaseNode):
        pets: RelationTo[Pet, RelationConfig(reverse_name="is_owned_by", validators=[MaxLen(2)])]
    ...
    maxlen_validator, relation_config, relation_identifier = Person.model_fields["pets"].metadata
    ...
    assert {'$ref': '#/$defs/PetReference'} in schema["properties"]["pets"]["items"]["anyOf"]
    assert {'$ref': '#/$defs/CrocodileReference'} in schema["properties"]["pets"]["items"]["anyOf"]
